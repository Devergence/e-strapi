...
export class StreamResolver {
  ...
  async (@Arg('streamId', () => ObjectIdScalar) streamId: ObjectId) {
  ...
  async deleteStream(
    ...
    const deleted = await StreamModel.findByIdAndDelete({ _id: streamId, author: ctx.res.locals.userId});
    if(!deleted) {
      throw new Error('Stream not found')
    }
    return true;
  ...
  async author(@Root() stream: Stream): Promise<User | null> {
    return UserModel.findById(stream.author);
  }
}
